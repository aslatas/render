...
{
    // NOTE(Dustin): Resizing is going to mess up changed state 
    SceneSettings* scene_settings = LoadSceneSettings("../../config/scene/default_scene.json");
    for (int i = 0; i < scene_settings->num_models; ++i)
    {
        SceneModelData model_model_to_add = scene_settings->model_data[i];

        Model_Separate_Data* model = (Model_Separate_Data*)malloc(sizeof(Model_Separate_Data));

        EModelLoadResult result = LoadGTLFModel(model_model_to_add, *model, GetPerDrawUniform(uniforms.object_count), 0, 0, uniforms.object_count);
        if (result == MODEL_LOAD_RESULT_SUCCESS) {
            uniforms.object_count++;
            AddToScene(*model);
        } else printf("FAILURE TO LOAD MODEL\n");
    }
    FreeSceneSettings(scene_settings);
