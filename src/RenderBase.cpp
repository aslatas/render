...
void InitializeVulkan(WindowInfo *window_info, VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info, BufferInfo *buffer_info)
{
    ...
    CreateSurface(window_info, vulkan_info);
    ChoosePhysicalDevice(vulkan_info);
    CreateLogicalDevice(vulkan_info);
    CreateSwapchain(window_info, vulkan_info, swapchain_info);
    CreateImageviews(vulkan_info, swapchain_info);
    CreateRenderpass(vulkan_info, swapchain_info);
    CreateDescriptorSetLayout(vulkan_info, swapchain_info);
    CreatePipeline(vulkan_info, swapchain_info);
    CreateFramebuffers(vulkan_info, swapchain_info);
    CreateCommandPool(vulkan_info, swapchain_info);
    CreateVertexBuffer(vulkan_info, swapchain_info, buffer_info);
    CreateIndexBuffer(vulkan_info, swapchain_info, buffer_info);
    CreateUniformBuffers(vulkan_info, swapchain_info, buffer_info);
    CreateDescriptorPool(vulkan_info, swapchain_info);
    CreateDescriptorSets(vulkan_info, swapchain_info, buffer_info);
    CreateCommandBuffers(vulkan_info, swapchain_info, buffer_info);
    CreateSyncPrimitives(vulkan_info, swapchain_info);
    
    window_info->is_initialized = true;
}

void CreateInstance(VulkanInfo *vulkan_info)
{
...
static void CreateSurface(WindowInfo *window_info, VulkanInfo *vulkan_info)
{
    ...
    create_info.hwnd = window_info->window;
...
void ChoosePhysicalDevice(VulkanInfo *vulkan_info)
{
...
void CreateLogicalDevice(VulkanInfo *vulkan_info)
{
...
static void ChooseSurfaceFormat(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info)
{
...
static void ChoosePresentMode(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info)
{
...
static void ChooseSwapchainExtent(WindowInfo *window_info, VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info)
{
    VkSurfaceCapabilitiesKHR capabilities;
    ...
    if (capabilities.currentExtent.width != 0xffffffff) {
        ...
        uint32_t width;
        uint32_t height;
        Win32GetClientRect(window_info, &width, &height);
...
void CreateSwapchain(WindowInfo *window_info, VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info) {
    ChooseSurfaceFormat(vulkan_info, swapchain_info);
    ChoosePresentMode(vulkan_info, swapchain_info);
    ChooseSwapchainExtent(window_info, vulkan_info, swapchain_info);
...
void CreateImageviews(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info)
{
...
void CreateRenderpass(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info) {
...
void CreateDescriptorSetLayout(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info) {
...
static VkShaderModule CreateShaderModule(VulkanInfo *vulkan_info, char *code, uint32_t length) {
...
void CreatePipeline(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info) {
    ...
    VkShaderModule vert_module = CreateShaderModule(vulkan_info, vert_code, vert_length);
    VkShaderModule frag_module = CreateShaderModule(vulkan_info, frag_code, frag_length);
...
void CreateFramebuffers(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info)
{
...
void CreateCommandPool(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info)
{
...
void CreateVertexBuffer(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info, BufferInfo *buffer_info)
{
    ...
    CreateBuffer(vulkan_info, buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, buffer_info->vertex_buffer, buffer_info->vertex_buffer_memory);
    
    CopyBuffer(vulkan_info, staging_buffer, buffer_info->vertex_buffer, buffer_size);
...
void CreateIndexBuffer(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info, BufferInfo *buffer_info)
{
    ...
    CreateBuffer(vulkan_info, buffer_size, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, buffer_info->index_buffer, buffer_info->index_buffer_memory);
    
    CopyBuffer(vulkan_info, staging_buffer, buffer_info->index_buffer, buffer_size);
    
    vkDestroyBuffer(vulkan_info->logical_device, staging_buffer, nullptr);
    vkFreeMemory(vulkan_info->logical_device, staging_buffer_memory, nullptr);
}

void CreateUniformBuffers(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info, BufferInfo *buffer_info)
{
    ...
    for (uint32_t i = 0; i < swapchain_info->image_count; ++i) {
        CreateBuffer(vulkan_info, buffer_size, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, buffer_info->uniform_buffers[i], buffer_info->uniform_buffers_memory[i]);
    }
}

void CreateDescriptorPool(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info)
{
...
void CreateDescriptorSets(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info, BufferInfo *buffer_info)
{
...
{
    VkBufferCreateInfo descriptor_info = {};
    descriptor_info.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO;
    descriptor_info.size = size;
    descriptor_info.usage = usage;
    descriptor_info.sharingMode = VK_SHARING_MODE_EXCLUSIVE;
    
    if (vkCreateBuffer(vulkan_info->logical_device, &descriptor_info, nullptr, &buffer) != VK_SUCCESS) {
...
void CreateCommandBuffers(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info, BufferInfo *buffer_info)
{
...
void CreateSyncPrimitives(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info) 
{
...
void DrawFrame(WindowInfo *window_info, VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info, BufferInfo *buffer_info) {
    ...
    if (result == VK_ERROR_OUT_OF_DATE_KHR) {
        RecreateSwapchain(window_info, vulkan_info, swapchain_info, buffer_info);
        return;
    ...
    UpdateUniforms(vulkan_info, swapchain_info, buffer_info, image_index);
...
void RecreateSwapchain(WindowInfo *window_info, VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info, BufferInfo *buffer_info) {
    uint32_t width;
    uint32_t height;
    if (!Win32GetClientRect(window_info, &width, &height)) {
    ...
    CleanupSwapchain(vulkan_info, swapchain_info);
    CreateSwapchain(window_info, vulkan_info, swapchain_info);
    CreateImageviews(vulkan_info, swapchain_info);
    CreateRenderpass(vulkan_info, swapchain_info);
    CreatePipeline(vulkan_info, swapchain_info);
...
void UpdateUniforms(VulkanInfo *vulkan_info, SwapchainInfo *swapchain_info, BufferInfo *buffer_info, uint32_t current_image) {
