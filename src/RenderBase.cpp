...
void InitializeRenderer()
{
    ...
    CreatePipelineInfos(pipeine_infos, material_count);
    swapchain_info.pipeline_layout = CreatePipelineLayout(
        ...
        infos[i].input_info.pVertexBindingDescriptions = &infos[i].binding_description;
        infos[i].input_info.pVertexAttributeDescriptions = &infos[i].attribute_descriptions;
        infos[i].viewport_info.pViewports = &infos[i].viewport;
        infos[i].viewport_info.pScissors = &infos[i].scissor;
        infos[i].blend_info.pAttachments = &infos[i].blend;
        
        VkGraphicsPipelineCreateInfo pipeline_info = {};
        pipeline_info.stageCount = infos[i].module_count;
        pipeline_info.pStages = infos[i].shader_stages;
        pipeline_info.pVertexInputState = &infos[i].input_info;
        pipeline_info.pInputAssemblyState = &infos[i].assembly_info;
        pipeline_info.pViewportState = &infos[i].viewport_info;
        pipeline_info.pRasterizationState = &infos[i].raster_info;
        pipeline_info.pMultisampleState = &infos[i].multisample_info;
        pipeline_info.pColorBlendState = &infos[i].blend_info;
        pipeline_info.pDepthStencilState = &infos[i].depth_stencil;
        pipeline_info.layout = layout;
        pipeline_info.renderPass = render_pass;
        pipeline_info.subpass = sub_pass;
        pipeline_info.basePipelineHandle = VK_NULL_HANDLE;
        VK_CHECK_RESULT(vkCreateGraphicsPipelines(vulkan_info.logical_device, VK_NULL_HANDLE, 1, pipeline_info, nullptr, &pipelines[i]));
        ...
        void CreatePipelineInfos(PipelineCreateInfo *pipeline_infos, Material *materials, uint32_t count, uint32_t layout)
        {
        for (uint32_t i = 0; i < material_count; ++i) {
        }
        pipeline_infos[0] = CreatePipelineInfo("shaders/vert.spv", "shaders/frag.spv", swapchain_info.pipeline_layout, swapchain_info.renderpass, 0);
        pipeline_infos[1] = CreatePipelineInfo("shaders/vert2.spv", "shaders/frag2.spv", swapchain_info.pipeline_layout, swapchain_info.renderpass, 0);
        pipeline_infos[2] = CreatePipelineInfo("shaders/stencil_vert.spv", nullptr, materials[2].layout, swapchain_info.renderpass, 0);
        pipeline_infos[2].raster_info.cullMode = VK_CULL_MODE_NONE;
        pipeline_infos[2].depth_stencil.depthTestEnable = VK_FALSE;
        pipeline_infos[2].depth_stencil.depthWriteEnable = VK_FALSE;
        pipeline_infos[2].depth_stencil.stencilTestEnable = VK_TRUE;
        pipeline_infos[2].depth_stencil.back = {};
        pipeline_infos[2].depth_stencil.back.compareOp = VK_COMPARE_OP_ALWAYS;
        pipeline_infos[2].depth_stencil.back.failOp = VK_STENCIL_OP_REPLACE;
        pipeline_infos[2].depth_stencil.back.depthFailOp = VK_STENCIL_OP_REPLACE;
        pipeline_infos[2].depth_stencil.back.passOp = VK_STENCIL_OP_REPLACE;
        pipeline_infos[2].depth_stencil.back.compareMask = 0xff;pipeline_infos[2].depth_stencil.back.writeMask = 0xff;
        pipeline_infos[2].depth_stencil.back.reference = 1;
        pipeline_infos[2].depth_stencil.front = pipeline_infos[2].depth_stencil.back;
