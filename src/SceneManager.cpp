...
RenderSceneMaterial* SceneManager::GetVisibleData(Camera::Camera *camera, glm::vec3 *camera_position)
{
    ...
    if (isOcclusion && camera_position != nullptr)
    {
        ...
        sm = scene->UpdateOcclusionVisibility(camera_position, camera, size);
        delete size;
    }
    else {
